#
# This file is auto-generated. DO NOT EDIT
# Generated by: autospec.py
#
Name     : audit-config
Version  : 1.0
Release  : 3
URL      : file:///home/clear/tar/audit-config-1.0.tar.gz
Source0  : file:///home/clear/tar/audit-config-1.0.tar.gz
Summary  : No detailed summary available
Group    : Development/Tools
License  : Apache-2.0
Requires: audit-config-data = %{version}-%{release}
Requires: audit
Patch1: 0001-add_makefile.patch

%description
No detailed description available

%package data
Summary: data components for the audit-config package.
Group: Data

%description data
data components for the audit-config package.


%prep
%setup -q -n audit-config-1.0
%patch1 -p1

%build
export http_proxy=http://127.0.0.1:9/
export https_proxy=http://127.0.0.1:9/
export no_proxy=localhost,127.0.0.1,0.0.0.0
export LANG=C.UTF-8
export SOURCE_DATE_EPOCH=1569602191
export GCC_IGNORE_WERROR=1
export AR=gcc-ar
export RANLIB=gcc-ranlib
export NM=gcc-nm
export CFLAGS="$CFLAGS -O3 -ffat-lto-objects -flto=4 "
export FCFLAGS="$CFLAGS -O3 -ffat-lto-objects -flto=4 "
export FFLAGS="$CFLAGS -O3 -ffat-lto-objects -flto=4 "
export CXXFLAGS="$CXXFLAGS -O3 -ffat-lto-objects -flto=4 "
make  %{?_smp_mflags}


%install
export SOURCE_DATE_EPOCH=1569602191
rm -rf %{buildroot}
## install_prepend content
%define debug_package %{nil}
%define _sysconfdir /usr/local/etc
%define license /usr/share/licenses/%{name}
## install_prepend end
install -d %{buildroot}%{_datadir}/starlingx
## install_append content
install -m640 syslog.conf %{buildroot}%{_datadir}/starlingx/syslog.conf
install -d -m 755 %{buildroot}%{license}
install -m 500 LICENSE %{buildroot}%{license}
%post
if [ $1 -eq 1 ] ; then
cp -f %{_datadir}/starlingx/syslog.conf %{_sysconfdir}/audisp/plugins.d/syslog.conf
chmod 640 %{_sysconfdir}/audisp/plugins.d/syslog.conf
fi
## install_append end

%files
%defattr(-,root,root,-)

%files data
%defattr(-,root,root,-)
/usr/share/licenses/audit-config/LICENSE
/usr/share/starlingx/syslog.conf
